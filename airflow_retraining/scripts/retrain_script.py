...
def run_full_pipeline():
    ...
    BASE_DATA_PATH = os.getenv("BASE_DATA_PATH", "/opt/airflow/data")
    MODEL_PATH = os.getenv("MODEL_PATH", "/opt/airflow/models")
    
    DEBUG_FRACTION = float(os.getenv("DEBUG_FRACTION", "0.05"))
    print(f"⚡️ DEBUG_FRACTION = {DEBUG_FRACTION:.0%}")

    # Абсолютные пути к данным
    RAW_EVENTS_PATH = os.path.join(BASE_DATA_PATH, "events.csv")
    RAW_ITEMS_PATH  = os.path.join(BASE_DATA_PATH, "items.parquet")
    ...
    item_popularity.to_parquet(os.path.join(BASE_DATA_PATH, "item_popularity.parquet"), index=False)
    similar_items.to_parquet(os.path.join(BASE_DATA_PATH, "similar_items.parquet"), index=False)
    ranked_recs[["visitorid", "itemid", "rank"]].to_parquet(
        os.path.join(BASE_DATA_PATH, "recommendations.parquet"), index=False
    )

    save_npz(os.path.join(MODEL_PATH, "user_item_train.npz"), user_item_train)
    ...
    with open(os.path.join(MODEL_PATH, "lgb_model.pkl"), "wb") as f:
